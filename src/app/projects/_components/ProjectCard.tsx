...
const ProjectCard = ({ project }: { project: Project }) => {
  return (
    <div key={project.name} className='bg-[#1e1e1f] border border-[#383838] rounded-xl p-5 flex flex-col justify-between transition duration-100 hover:-translate-y-2'>
      <div className='mb-6'>
        <div>
          <Image className='rounded-xl w-full ' src={project.img} alt='image' width={2000} height={2000} />
        </div>
      ...
      <div>
        ...
        <div className='flex justify-end items-end gap-2 text-white mt-2'>
          <div>
            <Link href={project.github} target='_blank' className='hover:text-sky-200'>
              <GithubIcon />
            </Link>
          </div>
          {project.url && (
            <div>
              <Link href={project.url} target='_blank' className='hover:text-sky-200'>
                <LinkToIcon />
              </Link>
            </div>
          )}
